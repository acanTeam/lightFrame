...
{
    /**
     * Default server settings assume the Light app is installed
    ...
    public function testMockEnvironment()
    {
        ...
        $env2 = Environment::getInstance();
        $this->assertSame($env, $env2);
        $this->assertInstanceOf('\Light\Mvc\Environment', $env);
     ...
     *
     * In earlier version, Environment would use PATH_INFO instead
     ...
     * the Light application environment. This test ensures the
    ...
    public function testHttps()
    {
        $_SERVER['HTTPS'] = 1;
        $env = Environment::getInstance(true);
        $this->assertEquals('https', $env['light.url_scheme']);
    }
    ...
    public function testNotHttps()
    {
        $_SERVER['HTTPS'] = '';
        $env = Environment::getInstance(true);
        $this->assertEquals('http', $env['light.url_scheme']);
    }
    ...
    {
        $env = Environment::getInstance(true);
        $this->assertEquals('', $env['light.input']);
    }
    ...
    {
        $env = Environment::getInstance(true);
        $this->assertTrue(is_resource($env['light.errors']));
    }
}
