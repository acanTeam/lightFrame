...
class Request
{
    ...
    /**
     * Is this an XHR request? (alias of Light_Http_Request::isAjax)
     * @return bool
     */
    ...
    {
        if (!isset($this->env['light.request.query_hash'])) {
            $output = array();
            if (function_exists('mb_parse_str') && !isset($this->env['light.tests.ignore_multibyte'])) {
                mb_parse_str($this->env['QUERY_STRING'], $output);
            } else {
                parse_str($this->env['QUERY_STRING'], $output);
            }
            $this->env['light.request.query_hash'] = Util::stripSlashesIfMagicQuotes($output);
        }
        if ($key) {
            if (isset($this->env['light.request.query_hash'][$key])) {
                return $this->env['light.request.query_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['light.request.query_hash'];
        }
    }
    ...
    {
        if (!isset($this->env['light.input'])) {
            throw new \RuntimeException('Missing light.input in environment variables');
        }
        if (!isset($this->env['light.request.form_hash'])) {
            $this->env['light.request.form_hash'] = array();
            if ($this->isFormData() && is_string($this->env['light.input'])) {
                $output = array();
                if (function_exists('mb_parse_str') && !isset($this->env['light.tests.ignore_multibyte'])) {
                    mb_parse_str($this->env['light.input'], $output);
                } else {
                    parse_str($this->env['light.input'], $output);
                }
                $this->env['light.request.form_hash'] = Util::stripSlashesIfMagicQuotes($output);
            } else {
                $this->env['light.request.form_hash'] = Util::stripSlashesIfMagicQuotes($_POST);
            }
        }
        if ($key) {
            if (isset($this->env['light.request.form_hash'][$key])) {
                return $this->env['light.request.form_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['light.request.form_hash'];
        }
    }

    /**
     * Fetch PUT data (alias for \Light\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch PATCH data (alias for \Light\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch DELETE data (alias for \Light\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    {
        $method = isset($this->env['light.method_override.original_method']) ? $this->env['light.method_override.original_method'] : $this->getMethod();
    ...
    {
        return $this->env['light.url_scheme'];
    }
    ...
    /**
     * LEGACY: Get Root URI (alias for Light_Http_Request::getScriptName)
     * @return string
     */
    ...
    /**
     * LEGACY: Get Resource URI (alias for Light_Http_Request::getPathInfo)
     * @return string
     */
