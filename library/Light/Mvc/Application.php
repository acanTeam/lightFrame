...
class Application
{
    ...
    public function __construct(array $configs = array())
    {
        ...
        $this->container->singleton('request', function ($c) {
            return new \Light\Http\Request($c['environment']);
        });
    ...
    /**
     * Set Light application name
     * @param  string $name The name of this Light application
     */
    ...
    *******************************************************************************/
    ...
    /**
     * Configure Light for a given mode
     *
     ...
     * only _after_ you initialize your Light app.
     *
     * @param  string $mode
    ...
    *******************************************************************************/
     ...
     * USAGE:
     *
     * Light::get('/foo'[, middleware, middleware, ...], callable);
     *
    ...
    public function get()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_GET, \Light\Http\Request::METHOD_HEAD);
    }
    ...
    public function post()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_POST);
    }
    ...
    public function put()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_PUT);
    }
    ...
    public function patch()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_PATCH);
    }
    ...
    public function delete()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_DELETE);
    }
    ...
    public function options()
    {
        ...
        return $this->mapRoute($args)->via(\Light\Http\Request::METHOD_OPTIONS);
    }
     ...
     *
     * If the $callable parameter is null, Light assumes you want
     ...
     * 2. When invoking the handler:
     *
     * If the $argument parameter is not callable, Light assumes you want
     * to invoke an already-registered handler. If the handler has been
    ...
    *******************************************************************************/
     ...
     *
     * This method declares the View to be used by the Light application.
     * If the argument is a string, Light will instantiate a new object
     * of the same class. If the argument is an instance of View or a subclass
     * of View, Light will use the argument as the View.
     *
     ...
     *
     * @param  string|\Light\View $viewClass The name or instance of a \Light\View subclass
     * @return \Light\View
     */
    public function view($viewClass = null)
    {
        if (!is_null($viewClass)) {
            ...
            if ($viewClass instanceOf \Light\View) {
                $this->view = $viewClass;
            } else {
    ...
    *******************************************************************************/
    ...
    public function lastModified($time)
    {
        if (is_integer($time)) {
            ...
            throw new \InvalidArgumentException('Light::lastModified only accepts an integer UNIX timestamp value.');
        }
    }
    ...
    public function etag($value, $type = 'strong')
    {
        ...
        if (!in_array($type, array('strong', 'weak'))) {
            throw new \InvalidArgumentException('Invalid Light::etag type. Expected "strong" or "weak".');
        }
    ...
    *******************************************************************************/
     ...
     * default Cookie setting values (set during Light::init) will be used instead.
     *
     * @param string    $name       The cookie name
     * @param string    $path       The path on the server in which the cookie will be available on
     * @param string    $domain     The domain that the cookie is available to
    ...
    *******************************************************************************/
    ...
    /**
     * Get the absolute path to this Light application's root directory
     *
     * This method returns the absolute path to the Light application's
     * directory. If the Light application is installed in a public-accessible
    ...
    *******************************************************************************/
    ...
    public function flash($key, $value)
    {
        if (isset($this->environment['light.flash'])) {
            $this->environment['light.flash']->set($key, $value);
        }
    }
    ...
    {
        if (isset($this->environment['light.flash'])) {
            $this->environment['light.flash']->now($key, $value);
        }
    }
    ...
    {
        if (isset($this->environment['light.flash'])) {
            $this->environment['light.flash']->keep();
        }
    }
    ...
    *******************************************************************************/
     ...
     * Run
     *
     * This method invokes the middleware stack, including the core Light application;
    ...
    public function run()
    {
        ...
        if (headers_sent() === false) {
            ...
            if (strpos(PHP_SAPI, 'cgi') === 0) {
                header(sprintf('Status: %s', \Light\Http\Response::getMessageForCode($status)));
            } else {
                header(sprintf('HTTP/%s %s', $this->config('http.version'), \Light\Http\Response::getMessageForCode($status)));
            }
    ...
    public function call()
    {
        try {
            ...
            ob_start();
            $this->applyHook('light.before.router');
            $dispatched = false;
            $matchedRoutes = $this->router->getMatchedRoutes($this->request->getMethod(), $this->request->getResourceUri());
            foreach ($matchedRoutes as $route) {
                try {
                    $this->applyHook('light.before.dispatch');
                    $dispatched = $route->dispatch();
                    $this->applyHook('light.after.dispatch');
                    if ($dispatched) {
                        break;
                    }
    ...
    *******************************************************************************/
     ...
     * these \ErrorException objects are then thrown and caught by Light's
